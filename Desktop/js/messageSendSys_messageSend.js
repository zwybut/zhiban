...
$(function(){
    ...
    //获取正文的高度
    $('.mainContentDiv').height($('.right').height() - $('.title').outerHeight() - $('.addressBox').outerHeight() - 20 - $('.customBox').outerHeight() - 18*2 - 70);
    $('#newInfo').height($('.mainContentDiv').height() - 40 - 4);
    ...
    $('.templateSelectBtn').click(function(){
        parent.newTemplateBoxShow();
        var parentdoc = parent.$('#addiframe').contents().find('#messageSend_addNewTemplate');
        parentdoc.find('#addUserBtn').click(function(){
            var content = $(this).attr('listid');
            $('#newInfo').val(content);
    ...
    $('#userList').tree({
        ...
        onCheck:function(node,checked){
            if(checked){
                ...
                sel_node = selId;
                groupName = node.text;
                if(selId.indexOf("top") >= 0 && node.children != null && node.children.length != 0){
                    selId = _note.children[0].id;
...
var sendMessageToUser = function(){
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if(noteMessageArr.length > 0){
                    var acceptTime = new Date().Format("yyyy-MM-dd hh:mm");
